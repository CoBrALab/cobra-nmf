...
def append_subjweights_plsstyle(df_demo,nmf_weights):
    ...
    for comp in range(0,maxrow):
        ...
        col='Comp'+str(comp+1)+'_T1T2'; m=0
        df_demo_nmf[col] = nmf_weights[comp,n_subjects*m:n_subjects*(m+1)]
        
        col='Comp'+str(comp+1)+'_DBM'; m=1
        df_demo_nmf[col] = nmf_weights[comp,n_subjects*m:n_subjects*(m+1)]
        
    print( "df had", len(df_demo.columns), "columns")
    print("numcomps is",  np.shape(nmf_weights)[0], "add", (2*maxrow), "columns")
    print("df_demo has", len(df_demo_nmf.columns), "columns")
    
    if ((len(df_demo_nmf.columns) - len(df_demo.columns)) == (2*maxrow)):
            return df_demo_nmf
    else:
        print("ERROR")
        return
...
H = hdf5storage.loadmat(nmf_res_filename)['H']
...
df_nmfweights_pls = df_sorted_nmfweights[comp_cols]
fname_pls_data="Hweights_pls_k10.csv" #MODIFY filename if you want
